...
void Crop::Apply(BmpHolder &image) const {
    ...
    image.header.size = (image.dibHeader.ImageWidth + ((4 - (image.dibHeader.ImageWidth * 3) % 4) % 4)) * image.dibHeader.ImageHeight * 3 + 54;
    image.dibHeader.ImageSize = image.header.size - 54;
}
